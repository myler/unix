...
int logger_size_check(char *logfile, int len)
{
    ...
    if(file_size + len < (MAX_LOG_BUFFER/2))
        return 0;
...
void logger(int level, const char *fmt, ...)
{
    ...
    */    
    ...
    {
        ret = logger_size_check(LOGFILE_1, logbuf_len);
        if(0 == ret)
        {
            fp = fopen(LOGFILE_1, "a+");
            if(NULL == fp)
                return;
        ...
        {
            ret = logger_size_check(LOGFILE_2, logbuf_len);
            if(0 == ret)
            {
                fp = fopen(LOGFILE_2, "a+");
                if(NULL == fp)
                    return;
...
#ifdef screen_log
	//logger(TLOG_NOTICE, "LOG:%s", logbuf);
	printf("LOG:%s", logbuf);
#endif
    fflush(fp);
    fclose(fp);
