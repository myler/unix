...
typedef struct tagD_var_string
{
    char var[_LEN_NORMAL];                                          // pgh 这个地方如果是移动侦测，需要设置为_LEN_DETECK_AREAS
}D_var_string;
...
typedef struct tagD_var_string_detect
{
    char var[_LEN_DETECK_AREAS];                                          // pgh 这个地方如果是移动侦测，需要设置为_LEN_DETECK_AREAS
}D_var_string_detect;
...
typedef struct tagD_var_week_string
{
    char var[_LEN_WEEK];        // 专用于布防时间的空间. 每周168个小时
}D_var_week_string;
...
{
    char username[_LEN_NORMAL];
    char password[_LEN_NORMAL];
}s_user_info;

typedef struct tag_web_login
{
...
AllConfigVar g_config_var;          // 全局变量,配置的所有参数
