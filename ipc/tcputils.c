...
read_sock (int sockhandle, unsigned char *buf, int length)
{
  ...
  if (byte_read > 0){
	...
	//logger(TLOG_NOTICE, "reste to read %d \n",mlength);
  }
  //logger(TLOG_NOTICE, "buffer value 0x%02X 0x%02X \n",buf[0],buf[1]);
	i++;
	//logger(TLOG_NOTICE, "waiting %d \n",i);
	if(i > 10000) return -1;
  } while (mlength > 0);
  return (mlength);
}
...
reportip( char *src, char *ip, unsigned short *port)
{
	...
	j=0;k=0;done=0;ipdone=0;
			...
			case '.':
				nbpt++;
				if(nbpt > 3){
//				logger(TLOG_NOTICE, "error fatal \n");
				return -1;
				}
			break;	
			case ':':
			...
			*port = (unsigned short) atoi (Ports);
			//printf ("Ip %s Port %s \n",AdIp,Ports);
			if(*port < 1024) {
//			logger(TLOG_NOTICE, "ERROR Set default port to 7070 \n");
			*port = 7070;
			}
return 0;
}
