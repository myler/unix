...
//#endif
{
    ...
    //logger(TLOG_NOTICE, "第一个storerecord的record[0]的BeginTime 应该是1-MM-DD HH:MM:SS, 实际呢？\r\n%s\r\n", ptest.storerecord[0].record[0].BeginTime);
    //logger(TLOG_NOTICE, "第一个storerecord的record[2]的BeginTime 应该空, 实际呢？\r\n%s\r\n", ptest.storerecord[0].record[2].BeginTime);
    //logger(TLOG_NOTICE, "第二个storerecord的record[1]的BeginTime 应该是4-MM-DD HH:MM:SS, 实际呢？\r\n%s\r\n", ptest.storerecord[1].record[1].BeginTime);

    //logger(TLOG_NOTICE, "\r\ndo 2------------------findxml_string\r\n");
    ret_err = findxml_string( Buff2, strlen(Buff2), "DevType", ret_string );
    //logger(TLOG_NOTICE,  "查找到的(string) DevType=%s\r\n", ret_string );

    //logger(TLOG_NOTICE, "\r\ndo 3------------------findxml_int\r\n");
    ret_err = findxml_int( Buff2, strlen(Buff2), "DevType", &n );
    //logger(TLOG_NOTICE,  "查找到的(int) DevType=%d\r\n", n );
    ...
    //logger(TLOG_NOTICE,  "利用上面struct数据，生成的xml为：\r\n%s\r\n", Buff2 );
    //logger(TLOG_NOTICE, "--------- 返回所需长度为 return len = %d,   实际生成的xml长度为 strlen( new_xml_buf ) = %d \r\n", ret_err, n );

    free( Buff2 );


    //logger(TLOG_NOTICE, "end------------------\n");
    return 0;
}
               ...
               int struct_index, __find_sub_strcut_desc fun )
{
        ...
        && fdarrayptr->inputmsg.name[0] != '\0' ) // 如果有<Message>这样的头时
    {
        if (outbuf)
        {
            sprintf( outbuf, XML_TITLE );
        }
        pf.write_buf_len += strlen(XML_TITLE);
    }
