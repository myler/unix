...
{
	int status;
	status = system("mount -t vfat /dev/mmc/blk0/part1 /ipc/ipc_data");
//	status = system("mount -t vfat -o rw,force /dev/mmc/blk0/part1 /ipc/ipc_data");

	if (status == -1){
    	logger(TLOG_ERROR, "execute mount error:%d-%s\n", errno, strerror(errno));
		return -1;
	}
    if (!WIFEXITED(status)){
   		logger(TLOG_ERROR, "mount run error:%d-%s\n", errno, strerror(errno));
		return -1;
    }

	if (WEXITSTATUS(status) != 0){
		logger(TLOG_NOTICE, "mount sd card failed:%d-%s\n", errno, strerror(errno));
		return -1;
	}

	g_config.store.sd_status.var = 1;
...
{
	int status;
	status = system("umount /ipc/ipc_data");
	if (status == -1){
    	logger(TLOG_ERROR, "execute umount error!");
		return -1;
	}
    if (!WIFEXITED(status)){
   		logger(TLOG_ERROR, "umount run error:%d-%s\n", errno, strerror(errno));
		return -1;
    }

    g_config.store.sd_status.var = 0;

    return 0;
}

int sd_mount(int count){
	while(count--){
		if(sd_action_insert() == 0 || g_config.store.sd_status.var == 1)
			return 0;
		sleep(3);
	}
...
int sd_umount(int count){
	while(count--){
		if(sd_action_remove() == 0 || g_config.store.sd_status.var == 0)
			return 0;
		sleep(3);
	}
...
int sd_init(int count)
{

	if(sd_umount(count) == -1){
		return -1;
	}
	...
	logger(TLOG_NOTICE, "sd_init:sd card mount success");
...
{
    logger(TLOG_NOTICE, "sd_detect_exit\n");
    if(-1== munmap( gpio_map, MMAP_LENGTH))
    {
        logger(TLOG_NOTICE, "mumap failed");
        return -1;
    }
    if(-1==close( fd ))
    {
        logger(TLOG_NOTICE, "close the file operator failed");
        return -1;
    }
    return 0;
}
    ...
    while(1)
    {
        ...
        if(sd_detect_operate() == 0)                    //SD卡存在
        {
            ...
            if(m!=g_config.para_store.SD_status.var)  //启动过程中有SD卡存在或程序在运行过程中有SD卡插入
            {
               ...
               logger(TLOG_NOTICE, "detect sd card insert!\n");
            }

            #ifdef debug_sd_detect_print
            logger(TLOG_NOTICE, "sd card is power on.\n");
            #endif
        }
        else                                          //SD卡不存在
        {
            ...
            if(m!=g_config.para_store.SD_status.var)  //程序在运行过程中SD卡被拔出
            {
                ...
                vs_sd_init();
                vs_set_save_base_folder();
                logger(TLOG_NOTICE, "SD卡被拔出\n");
            }
            #ifdef debug_sd_detect_print
            logger(TLOG_NOTICE, "sd card is power off.\n");
            #endif
        }
        sleep(1);
...
{
   if(sd_detect_init() != 0)
        logger(TLOG_NOTICE, "sd card detect init error!\n");
    sd_detect_wait();
    sd_detect_exit();
}
*/
